...
export class DataCollectionAppointmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getMeetingTitlesByPatientId();
  ...
  getMeetingTitlesByPatientId() {
    this.apptService.getMeetingTitlesByPatientId(this.data.user.patientId).subscribe(res => {
      this.meetingTitles = res;
    });
  }
