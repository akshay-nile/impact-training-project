...
@Component({
  ...
  templateUrl: './appointment-book-dialog.component.html',
  styleUrls: ['./appointment-book-dialog.component.css']
})
export class AppointmentBookDialogComponent implements OnInit {
  ...
  constructor(private snackbar: MatSnackBar,
    ...
    private dialogRef: MatDialogRef<AppointmentBookDialogComponent>,
    ...
    this.employeeName = this.data.user.title + '. ' + this.data.user.firstName + ' ' + this.data.user.lastName;
    ...
    this.appointmentForm = this.formBuilder.group({
      ...
      physician: [{value: this.data.user.employeeId, disabled:true}],
      employeeId: [{ value: this.data.user.employeeId, disabled: true }],
  ...
  showPatientId() {
    this.appointmentForm.controls['patientId'].setValue(this.appointmentForm.value.patient);
  }
  ...
  private getAppointmentReady(): any {
    ...
    for (let field of 'title=meetingTitle patientId=patientId description=description employeeId=physician date=aptDate'.split(' ')) {
  ...
  bookAppointment() {
    ...
    appointment['editHistory'] = "Booked by " + this.employeeName + " (Doctor) on " + this.appointmentForm.getRawValue().aptDate;
